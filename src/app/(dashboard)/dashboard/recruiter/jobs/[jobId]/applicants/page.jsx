...
export default function ViewApplicants() {
  ...
  const load = useCallback(async () => { try { const { data } = await authClient.getSession(); setRows((await api.getJobApplications(jobId, data?.session?.token)).data || []); } catch (e) { toast.error(e.message || "Unable to load applicants"); } finally { setLoading(false); } }, [jobId]);
