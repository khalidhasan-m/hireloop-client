  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data, error: signUpError } = await authClient.signUp.email({
        ...
        role: formData.role === "recruiter" ? "recruiter" : "seeker",
        callbackURL: dashboardPathForRole(formData.role),
      });
      ...
      if (data) {
        ...
        // Prefer role from signup response; fall back to session
        let role = data?.user?.role || formData.role;
        if (!data?.user?.role) {
          try {
            const sessionRes = await authClient.getSession();
            role = sessionRes?.data?.user?.role || formData.role;
          } catch (_) {
            /* use formData.role */
          }
        }
        ...
        <div className="w-full max-w-115">
          ...
          <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-[#0b0b0f]/90 p-5 shadow-[0_25px_80px_rgba(0,0,0,.45)] backdrop-blur-xl sm:p-7">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {formData.password && (
                  <div className="mt-2">
                    ...
                    <div className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1">
                      ...
                      <PasswordRule passed={passwordRules.uppercase} text="Uppercase letter" />
                      <PasswordRule passed={passwordRules.lowercase} text="Lowercase letter" />
                      <PasswordRule passed={passwordRules.number} text="Number" />
                      <PasswordRule passed={passwordRules.special} text="Special character" />
                    </div>
                  </div>
                )}
              </div>
              ...
              <div>
                <label htmlFor="confirmPassword" className="mb-1.5 block text-xs font-medium text-gray-300">
                  Confirm password
                </label>
                <div className="relative">
                  <HiLockClosed className="pointer-events-none absolute left-3 top-1/2 z-10 -translate-y-1/2 text-sm text-gray-600" />
